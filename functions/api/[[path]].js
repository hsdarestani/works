...
async function updateTask(db, taskId, body) {
  ...
  let title = body?.title === undefined ? existing.title : String(body.title || "").trim();
...
async function createComment(db, body) {
  ...
  if (parentId !== null) {
    ...
    if (!parent || Number(parent.task_id) !== taskId) return json({ error: "Parent comment not found for this task." }, 400);
  }
